package org.fantasygold.wallet.ui.fragment.pin_fragment;
...
import org.fantasygold.wallet.utils.CryptoUtilsCompat;
import org.fantasygold.wallet.utils.PinEntryEditText;
import org.fantasygold.wallet.utils.ThemeUtils;
import org.fantasygold.wallet.datastorage.FantasyGoldSharedPreference;
import org.fantasygold.wallet.utils.CryptoUtils;
...
public class PinDialogFragment extends DialogFragment {
    ...
    @BindView(org.fantasygold.wallet.R.id.til_wallet_pin)
    PinEntryEditText mWalletPin;

    @BindView(org.fantasygold.wallet.R.id.tv_toolbar_title)
    FontTextView mTextViewToolBarTitle;

    @BindView(org.fantasygold.wallet.R.id.tooltip)
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        View view = LayoutInflater.from(getActivity()).inflate(ThemeUtils.getCurrentTheme(getContext()).equals(ThemeUtils.THEME_DARK) ? org.fantasygold.wallet.R.layout.dialog_fragment_pin : org.fantasygold.wallet.R.layout.dialog_fragment_pin_light, null);
    ...
    private String getSixDigitPassword() {
        return FantasyGoldSharedPreference.getInstance().getSixDigitPassword(getContext());
    }
    ...
    private String getFourDigitPassword() {
        return FantasyGoldSharedPreference.getInstance().getPassword(getContext());
    }
            ...
            ((MainActivity) getActivity()).checkSensorState(new MainActivity.SensorStateListener() {
                ...
                public void onRequest(MainActivity.SensorState sensorState) {
                    if (sensorState == MainActivity.SensorState.READY) {
                        mTextViewToolBarTitle.setText(org.fantasygold.wallet.R.string.confirm_fingerprint_or_pin);
                        prepareSensor();
                    } else {
                        mTextViewToolBarTitle.setText(org.fantasygold.wallet.R.string.enter_pin);
                    }
                }
            });
        } else {
    ...
    private void confirmError(String errorText) {
        if (getDialog() != null) {
            ...
            tooltip.setTextColor(ContextCompat.getColor(getContext(), org.fantasygold.wallet.R.color.accent_red_color));
    ...
    private class FingerprintHelper extends FingerprintManagerCompat.AuthenticationCallback {
        ...
        public void onAuthenticationSucceeded(FingerprintManagerCompat.AuthenticationResult result) {
            ...
            String encoded = FantasyGoldSharedPreference.getInstance().getTouchIdPassword(getContext());
    ...
    private void confirm(String pin) {
        Long banTime = FantasyGoldSharedPreference.getInstance().getBanTime(getContext());
        ...
        if (isCorrect) {
            ...
            confirmError(getContext().getString(org.fantasygold.wallet.R.string.incorrect_pin));
        }
    }
    ...
    private void changeBanState(boolean isCorrect) {
        ...
        if (isCorrect) {
            ...
            failedAttemptsCount = FantasyGoldSharedPreference.getInstance().getFailedAttemptsCount(getContext());
            failedAttemptsCount++;
            if (failedAttemptsCount == 3) {
                Long currentTime = Calendar.getInstance().getTimeInMillis();
                Long banTime = currentTime + mBanTime;
                FantasyGoldSharedPreference.getInstance().setBanTime(getContext(), banTime);
                failedAttemptsCount = 0;
                mPinCallBack.onError(getString(R.string.sorry_please_try_again_in) + " " + mBanTime.intValue() / 60000 + " " + getString(R.string.minutes));
            }
        }
        FantasyGoldSharedPreference.getInstance().setFailedAttemptsCount(getContext(), failedAttemptsCount);
    }
