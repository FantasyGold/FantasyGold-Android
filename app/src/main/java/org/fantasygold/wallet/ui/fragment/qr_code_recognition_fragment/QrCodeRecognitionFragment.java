package org.fantasygold.wallet.ui.fragment.qr_code_recognition_fragment;
...
public class QrCodeRecognitionFragment extends Fragment implements ZXingScannerView.ResultHandler {
    ...
    @BindView(org.fantasygold.wallet.R.id.camera_container)
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(org.fantasygold.wallet.R.layout.fragment_qrcode_camera, container, false);
    }
    ...
    public void handleResult(Result result) {
        ...
        Pattern pattern = Pattern.compile("fantasygold:(.*?)\\?");
        Matcher matcher = pattern.matcher(result.getText());
        if (matcher.find()) {
            receiveAddr = matcher.group(1);
        } else {
            pattern = Pattern.compile("fantasygold:(.*?)$");
        ...
        if (!TextUtils.isEmpty(receiveAddr)) {
            ...
            pattern = Pattern.compile("^$|^[fF][a-km-zA-HJ-NP-Z1-9]{0,33}$");
            matcher = pattern.matcher(result.getText());
            if (matcher.matches()) {
