...
import static org.fantasygold.wallet.utils.crypto.KeyStoreHelper.trimEndSpaces;
...
class PinInteractorImpl implements PinInteractor {
    ...
    private String getFourDigitPassword() {
        String encryptedPinHash = FantasyGoldSharedPreference.getInstance().getPassword(mContext);
        return KeyStoreHelper.decrypt(FGC_PIN_ALIAS, encryptedPinHash);
    }

    @Override
    public String getSixDigitPassword() {
        return FantasyGoldSharedPreference.getInstance().getSixDigitPassword(mContext);
    }

    @Override
    public Integer getFailedAttemptsCount() {
        return FantasyGoldSharedPreference.getInstance().getFailedAttemptsCount(mContext);
    }

    @Override
    public Long getBanTime() {
        return FantasyGoldSharedPreference.getInstance().getBanTime(mContext);
    }

    @Override
    public void setFailedAttemptsCount(int failedAttemptsCount) {
        FantasyGoldSharedPreference.getInstance().setFailedAttemptsCount(mContext, failedAttemptsCount);
    }

    @Override
    public void setBanTime(long banTime) {
        FantasyGoldSharedPreference.getInstance().setBanTime(mContext, banTime);
    }

    @Override
    public void saveSixDigitPassword(String password) {
        FantasyGoldSharedPreference.getInstance().saveSixDigitPassword(mContext, password);
    }
    ...
    public void savePassphraseSaltWithPin(String pin, String passphrase) {
        ...
        FantasyGoldSharedPreference.getInstance().saveSeed(mContext, encryptedSaltPassphrase);
    }

    @Override
    public byte[] getSaltPassphrase() {
        String encryptedSaltPassphrase = FantasyGoldSharedPreference.getInstance().getSeed(mContext);
        return Base64.decode(encryptedSaltPassphrase, Base64.DEFAULT);
    }

    @Override
    public String getTouchIdPassword() {
        return FantasyGoldSharedPreference.getInstance().getTouchIdPassword(mContext);
    }

    @Override
    public void saveTouchIdPassword(String password) {
        FantasyGoldSharedPreference.getInstance().saveTouchIdPassword(mContext, password);
    }
    ...
    public void setKeyGeneratedInstance(boolean isKeyGenerated) {
        FantasyGoldSharedPreference.getInstance().setKeyGeneratedInstance(mContext, isKeyGenerated);
    }
