...
import org.fantasygold.wallet.ui.activity.splash_activity.SplashActivity;
import org.fantasygold.wallet.ui.base.base_activity.BaseActivity;
...
import org.fantasygold.wallet.ui.fragment.wallet_fragment.WalletFragment;
import org.fantasygold.wallet.ui.fragment.wallet_main_fragment.WalletMainFragment;
import org.fantasygold.wallet.utils.CustomContextWrapper;
import org.fantasygold.wallet.utils.FontManager;

import org.fantasygold.wallet.ui.fragment.profile_fragment.ProfileFragment;
...
import static org.fantasygold.wallet.ui.base.base_fragment.BaseFragment.BACK_STACK_ROOT_TAG;
...
public class MainActivity extends BaseActivity implements MainActivityView {
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (intent != null && intent.getAction() != null) {
            switch (intent.getAction()) {
                case FantasyGoldIntent.OPEN_FROM_NOTIFICATION:
                    openRootWallet();
                    break;
                default:
                    break;
    ...
    public boolean checkTouchIdEnable() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (FantasyGoldSharedPreference.getInstance().isTouchIdEnable(getContext())) {
    ...
    public void initializeViews() {
        ...
        switch (intent.getAction()) {
            case FantasyGoldIntent.SEND_FROM_SDK:
                getPresenter().setSendFromIntent(true);
                mAddressForSendAction = intent.getStringExtra(FantasyGoldIntent.SEND_ADDRESS);
                mAmountForSendAction = intent.getStringExtra(FantasyGoldIntent.SEND_AMOUNT);
                mTokenAddressForSendAction = intent.getStringExtra(FantasyGoldIntent.SEND_TOKEN);
                break;
            case NfcAdapter.ACTION_NDEF_DISCOVERED:
                getPresenter().setSendFromIntent(true);
                mAddressForSendAction = "FBShaLBf1nAX3kznmGU7vM85HFRYJVG6ut";
    ...
    protected void attachBaseContext(Context newBase) {
        super.attachBaseContext(CustomContextWrapper.wrap(newBase, FantasyGoldSettingSharedPreference.getInstance().getLanguage(newBase)));
    }

    public FantasyGoldApplication getFantasyGoldApplication() {
        return (FantasyGoldApplication) getApplication();
    }
    ...
    public Realm getRealm(){
        return ((FantasyGoldApplication)getApplication()).getRealm();
    }
}