...
public class FantasyGoldSettingSharedPreference {

    private final String FGC_SETTING = "fantasygold_setting";
    ...
    private final String FGC_LANGUAGE = "fantasygold_language";

    private List<LanguageChangeListener> mLanguageChangeListeners;
    private static FantasyGoldSettingSharedPreference sInstance = null;

    public static FantasyGoldSettingSharedPreference getInstance() {
        if (sInstance == null) {
            sInstance = new FantasyGoldSettingSharedPreference();
        }
        return sInstance;
    }

    private FantasyGoldSettingSharedPreference(){
        mLanguageChangeListeners = new ArrayList<>();
    }

    public void setShowContractsDeleteUnsubscribeWizard(Context context, boolean isShow) {
        SharedPreferences mSharedPreferences = context.getSharedPreferences(FGC_SETTING, Context.MODE_PRIVATE);
    ...
    public boolean getShowContractsDeleteUnsubscribeWizard(Context context) {
        return context.getSharedPreferences(FGC_SETTING, Context.MODE_PRIVATE).getBoolean(SHOW_CONTRACTS_DELETE_UNSUBSCRIBE_WIZARD, true);
    }
    ...
    public int getCodeVersion(Context context){
        return context.getSharedPreferences(FGC_SETTING, Context.MODE_PRIVATE).getInt(MIGRATION_VERSION_STRING, 0);
    }
    ...
    public String getFeePerKb(Context context) {
        return context.getSharedPreferences(FGC_SETTING, Context.MODE_PRIVATE).getString(FEE_PER_KB, "0.006");
    }

    public String getLanguage(Context context) {
        return context.getSharedPreferences(FGC_SETTING, Context.MODE_PRIVATE).getString(FGC_LANGUAGE, "us");
    }

    public void saveLanguage(Context context, String language) {
        SharedPreferences mSharedPreferences = context.getSharedPreferences(FGC_SETTING, Context.MODE_PRIVATE);
        SharedPreferences.Editor mEditor = mSharedPreferences.edit();
        mEditor.putString(FGC_LANGUAGE, language);
        mEditor.apply();
