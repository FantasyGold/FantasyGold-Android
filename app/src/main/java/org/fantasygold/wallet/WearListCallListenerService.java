...
import static org.fantasygold.wallet.FantasyGoldApplication.REALM_NAME;
...
public class WearListCallListenerService extends WearableListenerService implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, MessageApi.MessageListener {
    ...
    public void onMessageReceived(MessageEvent messageEvent) {
        ...
        if(messageEvent.getPath().contains("/get_history")) {
            ...
            currentAddress = FantasyGoldSharedPreference.getInstance().getCurrentAddress(getApplicationContext());
            sendData();
        } else {
            stopSelf();
        }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    final String balance = FantasyGoldSharedPreference.getInstance().getBalanceString(WearListCallListenerService.this);
                    final String uncBalance = FantasyGoldSharedPreference.getInstance().getUnconfirmedBalanceString(WearListCallListenerService.this);
                    final String address = FantasyGoldSharedPreference.getInstance().getCurrentAddress(WearListCallListenerService.this);
                    Gson gson = new Gson();
