package org.fantasygold.wallet.utils.migration_manager;
...
import static org.fantasygold.wallet.utils.crypto.KeyStoreHelper.FGC_PIN_ALIAS;
...
public class MigrationManager {
    ...
    private static KeystoreMigrationResult migratePassphrase(Context context) {
        if (TextUtils.isEmpty(FantasyGoldSharedPreference.getInstance().getKeystoreMigrationState(context))) {
            String encryptedSaltPassphrase = FantasyGoldSharedPreference.getInstance().getSeed(context);
            if (!TextUtils.isEmpty(encryptedSaltPassphrase)) {
                try {
                    KeyStoreHelper.createKeys(context, FGC_PIN_ALIAS);
                    byte[] bytes = KeyStoreHelper.decryptToBytes(FGC_PIN_ALIAS, encryptedSaltPassphrase);
                    savePassphraseSalt(context, bytes);
                    FantasyGoldSharedPreference.getInstance().setKeyStoreMigrationState(context, KeystoreMigrationResult.SUCCESS.name());
                    return KeystoreMigrationResult.SUCCESS;
                } catch (NoSuchProviderException | NoSuchAlgorithmException | InvalidAlgorithmParameterException | RuntimeException e) {
                    return KeystoreMigrationResult.ERROR;
                }
            } else {
                FantasyGoldSharedPreference.getInstance().setKeyStoreMigrationState(context, KeystoreMigrationResult.NOT_NEED.name());
                return KeystoreMigrationResult.NOT_NEED;
            }
        } else {
            return KeystoreMigrationResult.NOT_NEED;
    ...
    private static void migratePinCode(Context context) {
        boolean isSixDigit;
        String encryptedPinHash = FantasyGoldSharedPreference.getInstance().getSixDigitPassword(context);
        isSixDigit = !TextUtils.isEmpty(encryptedPinHash);
        if (!isSixDigit) {
            encryptedPinHash = FantasyGoldSharedPreference.getInstance().getPassword(context);
            if (TextUtils.isEmpty(encryptedPinHash)) {
                return;
            }
        }
        String decrypt = KeyStoreHelper.decrypt(FGC_PIN_ALIAS, encryptedPinHash);
        if (isSixDigit) {
            FantasyGoldSharedPreference.getInstance().saveSixDigitPassword(context, decrypt);
        } else {
            FantasyGoldSharedPreference.getInstance().savePassword(context, decrypt);
        }
    }
    ...
    private static void savePassphraseSalt(Context context, byte[] saltPassphrase) {
        ...
        FantasyGoldSharedPreference.getInstance().saveSeed(context, base64);
    }

    private void clearWallet(Context context, Realm realm) {
        FantasyGoldSharedPreference.getInstance().forceClear(context);
